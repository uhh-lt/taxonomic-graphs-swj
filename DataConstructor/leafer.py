...
class Leafer:
    ...
    def split_train_test(
        ...
        self.collector = Collector(
            ...
            min_to_test_rate,
            weights=weights,
            use_1sense_hypernyms=use_1sense_hypernyms,
        )
        ...
        if to_test_rate == 1:
            if self.goes_to_test():
                if self.divide_on_half():
                    ...
                    # put nodes in test to use in prompt 
                    ...
                    )  # predict child from parent in train 
                ...
                if self.divide_on_half():
                    # when brothers predicted inside train
                    # model should see such examples
                    elem["case"] = "only_leafs_divided"
                    # делим пополам
                    to_test_subset = children[: len(children) // 2]
                    to_train_subset = children[len(children) // 2 :]
